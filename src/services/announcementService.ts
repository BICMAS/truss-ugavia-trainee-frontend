...
export const getAnnouncements = async () => {
  try {
    ...
    const { isDemoSession, getDemoAnnouncements } = await import("@/demo");
...
export const showAnnouncementNotification = async (message: string) => {
  ...
  try {
    if (
      ...
      new Notification("Truss Ugavi Announcement", {
        body: message,
        icon: "/logo.png",
        badge: "/logo.png",
    ...
    await registration.showNotification("Truss Ugavi Announcement", {
      body: message,
      icon: "/logo.png",
      badge: "/logo.png",
      tag: "bicmas-announcement",
