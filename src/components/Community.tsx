        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
          <div className="p-4 border-b border-slate-100 flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                 <span className="bg-brand-yellow/20 text-brand-green text-[10px] font-bold px-2 py-0.5 rounded uppercase">{selectedThread.category}</span>
             ...
             <div className="flex gap-4">
               ...
               <div className="flex-1 space-y-2">
                  ...
                  <div className="flex gap-4 pt-2">
                     <button className="flex items-center gap-1 text-slate-500 hover:text-brand-green text-sm"><ThumbsUp size={16} /> {selectedThread.likes}</button>
                     <button className="flex items-center gap-1 text-slate-500 hover:text-brand-green text-sm"><MessageSquare size={16} /> Reply</button>
                  </div>
               </div>
             </div>
          ...
          <div className="p-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
            <div className="flex gap-2">
              ...
              <button className="brand-btn-secondary p-2"><Send size={20} /></button>
            </div>
          </div>
        </div>
      );
      ...
      <div className="space-y-4 animate-in fade-in zoom-in duration-300">
        <div className="flex items-center justify-between">
           ...
           <button 
             ...
             className="brand-btn-secondary px-4 py-2 text-sm flex items-center gap-2"
           >
        ...
        <div className="grid gap-4">
           {threads.filter(t => forumFilter === 'All' || t.category === forumFilter).map(thread => (
             <div 
               ...
               className="brand-card p-5 hover:border-brand-green/20 hover:shadow-md transition-all cursor-pointer group"
             >
                <div className="flex justify-between items-start mb-2">
                   <div className="flex items-center gap-2">
                      <img src={thread.author.avatar} alt="" className="w-8 h-8 rounded-full" />
                      <div>
                        <span className="block text-sm font-semibold text-slate-900 group-hover:text-brand-green transition-colors">{thread.title}</span>
                        <span className="text-xs text-slate-500">{thread.author.name} • {thread.timestamp}</span>
                      </div>
                   </div>
                   {thread.isPinned && <Pin size={16} className="text-brand-yellow fill-current rotate-45" />}
                </div>
            ...
            <div className="flex-1 overflow-y-auto">
               {MOCK_CONTACTS.map(contact => (
                 ...
                 >
                    ...
                    {contact.unread > 0 && (
                      <div className="w-5 h-5 bg-brand-green rounded-full text-white text-[10px] flex items-center justify-center font-bold">
                        {contact.unread}
                      </div>
                    )}
                 </button>
              ...
              <>
                ...
                <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-slate-100">
                   <div className="flex gap-2">
                     ...
                     />
                     <button type="submit" className="brand-btn-secondary p-3">
                       <Send size={20} />
                     </button>
                   </div>
                </form>
           ...
           <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg relative z-10 overflow-hidden animate-in zoom-in-95 duration-200">
              ...
              <form onSubmit={handlePostTopic} className="p-6 space-y-4">
                 ...
                 <div className="flex gap-3 pt-2">
                    ...
                    <button 
                      type="submit" 
                      className="flex-1 py-2.5 brand-btn-primary font-medium shadow-sm"
                    >
                      Post Discussion
                    </button>
                 </div>
